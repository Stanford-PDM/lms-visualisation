...
object materialize {
  ...
  object Column {
    private def column(prefix: String, size: Int, offset: Int, cls: Seq[String]) = offset match {
      ...
      case n => Div(classes = Seq("col", s"$prefix$size", s"offset-$prefix$n") ++ cls)
    }
    def small(size: Int, offset: Int = 0, classes: Seq[String] = Seq.empty) = column("s", size, offset, classes)
    def medium(size: Int, offset: Int = 0, classes: Seq[String] = Seq.empty) = column("m", size, offset, classes)
    def large(size: Int, offset: Int = 0, classes: Seq[String] = Seq.empty) = column("l", size, offset, classes)
  }
